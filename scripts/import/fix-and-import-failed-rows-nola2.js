...
async function fixAndImportFailedRows() {
  ...
  fs.createReadStream('nola2_past_5_years.csv')
    ...
    .on('end', async () => {
      console.log(`Processing ${results.length} rows with fixes...`);
      ...
      for (let i = 0; i < results.length; i += batchSize) {
        ...
        try {
          ...
          batch.forEach((row, index) => {
            ...
            // Fix long descriptions by truncating to 1000 characters
            const truncatedDescription = row.description && row.description.length > 1000 
          ...
          for (const row of batch) {
            try {
              ...
              errorCount++;
              console.error(`Failed to import individual row ${row.numstring}:`, innerError.message);
            }
          }
        }
      }
      ...
      console.log(`Expected match: ${successCount === results.length ? 'YES' : 'NO'}`);
